...
> | 전체 마비 `MASS_STUN` | 9초마다 **살아 있는 적 전부** 기절 + 번쩍임. `CRunSkillEffect_MassStun` → `IRunSkillHost.Stun_AllEnemies` | 기절 시간(1초 + 0.3/Lv) |
>
...
- **한 번의 점령이 맵 전체에서 차지하는 비율**로 배율 구간을 나눈다(`CaptureRewardInfo.csv`,
  현재 0~10% 1.0배 / 10~20% 1.2배 / 20~35% 2배 / 35~50% 3배 / 50%~ 5배 — 전부 기획이 CSV에서
  바로 조정한다). 표가 없으면 1배로 지급해 게임이 죽지 않는다
...
|---|---|---|
| 4방향(현재) | — | 기준점 |
| 8방향 대각선(느림) | **가능** | `MOVE_DIR`에 대각 4개 추가, `Get_NextCell`(2-11-1 "어디로든 신발"이 정리해 둔 그 함수)에 대각 좌표만 더하면 이동 자체는 된다. 다만 두 가지를 반드시 같이 정해야 한다 — ① **코너 컷팅**: 대각선의 양옆 두 칸이 전부 벽/트레일/몬스터일 때 그 사이로 빠져나가게 둘지 막을지(`Step_To` 판정에 영향), ② **속도 보정**: 대각선 한 칸이 직선 한 칸보다 √2배 더 간다 — 초당 이동 칸수를 그대로 두면 대각선이 사실상 더 빠른 지름길이 되어버려 "느리다"는 컨셉과 반대로 간다. 시간 보정을 넣어야 진짜 페널티가 된다 |
