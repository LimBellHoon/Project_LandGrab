...
```
CStage_Manager.Ignite_Fuse   몬스터의 몸이 트레일에 처음 닿은 칸에서 발화. 이미 타는 중이면 새로 안 붙는다
CStage_Manager.Tick_Fuse     매 프레임 발화 지점부터 태운다(FUSE_SPEED_CELL_PER_SEC, 초당 3칸)
CTerritoryGrid.Burn_TrailCell 태운 칸만 도로 빈 땅으로 — 목록 순서는 유지한다(다음 칸을 계속 찾아야 해서)
CTerritoryGrid.Step_To       IS_TRAIL_BURNING이 켜져 있으면 안전 지대로 돌아와도 점령하지 않는다
```

- **발화는 몸(`HIT_RANGE`)이 트레일에 닿는 순간**(`CTerritoryGrid.Try_Find_StateWithin` — 기존
  `Is_StateWithin`을 닿은 칸까지 돌려주도록 확장했다, 1-1)이다. 몸 판정과 같은 자리라 "화면에서는
  닿았는데 안 붙었다"가 안 생긴다
- **발화 지점의 몬스터 공격력(`EnemyInfo.iAttack`)을 그 자리에서 기억해 둔다**(`m_iFuseDamage`) —
  불이 다 타들어올 때쯤엔 그 몬스터가 어디 있는지, 심지어 살아 있는지조차 알 수 없어서다
- **트레일은 `List<int>` 순서를 그대로 유지한 채 칸 상태만 지운다.** 목록에서 지우면(shift) 도화선이
  "몇 번째까지 탔는지" 추적하던 인덱스가 전부 밀린다 — 상태만 `EMPTY`로 돌리는 쪽이 훨씬 단순하다.
  덕분에 마스크 렌더러가 이미 쓰고 있는 더티 셀 갱신(`Set_CellDirty`)을 그대로 타서, **불이 타들어오는
  모습 자체가 새 그리기 코드 없이 "선이 지나온 순서대로 사라지는" 것만으로 표현된다**
...
- 발화 지점을 알릴 자리로 `CStage_Manager.OnFuseIgnited`(칸 좌표)를 훅만 뚫어 뒀다 — 스파크 이펙트는
  ...
  지금도 마스크 렌더러가 태운 칸을 지워 주므로 **연출 없이도 기능은 이미 동작한다**
