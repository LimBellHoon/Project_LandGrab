...
- 260920_**선을 길게 그을수록 시야가 넓어진다**(`CCameraTrailZoom`). 멀리 나갔을 때 돌아올 길과
  쫓아오는 몬스터를 같이 봐야 판단이 되는데, 시야가 고정이면 화면 밖에서 잘려 억울하게 죽는다.
  트레일 한 칸당 `m_fTrailZoomPerCell`(1.2%)씩 넓어지고 `m_fTrailZoomMax`(1.6배)에서 멈춘다.
  선을 거두면(점령 · 사망) 1배로 돌아온다. **펀치줌과 같은 자리에서 크기에 곱하므로 둘이 서로를 몰라도 겹친다**(2-10-3).
  한 칸 늘 때마다 튀지 않게 `SmoothDamp`로 따라붙는다 — 판정은 모듈, 적용은 `Tick_Camera`다(화면 없이 테스트)
...
> 나선형 · 관성형은 아직 설계만 있다(2-19). 그쪽은 "다음 칸 = 현재 + 입력"이라는 전제를 갈아엎는
> 작업이라 8방향과 공수가 다르다.
