...
└── Script/
    ...
    │                       CPolygon_Utility(260923_다각형 판정 헬퍼)
...
- `CGridRenderer`는 점령지 다각형을 마스크 텍스처에 찍어 가림막을 뚫는다. **셰이더 없음.**
  ...
  **선은 텍스처가 아니라 `LineRenderer`로 따로 그린다**(굵기 0.55칸, 타는 구간은 빼고 불꽃 머리를 붙인다). 자세한 건 2-5 참고.
...
- 선을 긋는 동안: 4방향 · 8방향은 누른 방향으로 곧게, 나선형은 아래 나선으로 방향을 틀며 간다.
  **뒤로 꺾는 입력(120도 넘게 어긋남)은 무시하고 선다** — 자기 선을 밟는 즉사를 막는다
