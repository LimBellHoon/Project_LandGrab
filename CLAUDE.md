...
**웨이브를 넘겨도 판을 새로 깔지 않는다**(260912). 점령한 칸을 그대로 두고 두 장만 갈아 끼우므로,
이미 뚫어 둔 구멍으로 다음 장이 비친다 — 같은 자리를 **겹겹이 벗겨 나가는** 모양이다.
그래서 `strWaveClearRatio`가 웨이브마다 올라간다(0.6 / 0.65 / 0.7). **누적 비율이기 때문이다.**
...
#### 웨이브 전환은 끊기지 않는다 (260912)
예전에는 가림막을 걷어 보여주고(REVEAL) 잠깐 두었다가(HOLD) 다시 덮는(COVER) 연출이 있었다.
**연출과 판 초기화를 함께 걷어냈다** — 웨이브가 넘어가도 플레이가 끊기지 않는 쪽을 골랐다.

지금은 `Next_Wave`가 바로 `Enter_Wave`를 부른다.

- 그리드를 `Reset`하지 않는다 → 점령한 칸이 유지된다
- 플레이어를 다시 놓지 않는다 → 있던 자리에서 계속 움직인다
- 몬스터·투사체·거미줄만 새 웨이브 구성으로 갈아 끼운다(`Spawn_Enemies`가 `Collect_Enemies`부터 한다)
- 액터를 세우지 않는다 → 제한 시간만 새 웨이브 값으로 바뀐다

마지막 웨이브를 넘기면 그 자리에서 바로 `STAGE_STATE.CLEAR`다.
