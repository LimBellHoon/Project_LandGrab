...
### 2-3-1. 선 스타일 — 색 · UV · 셰이더 (260924)
선은 띠 메시라 **겉모습을 마음대로 입힐 수 있다.** `TrailRenderer`로 바꾸자는 안이 나왔지만 쓰지 않았다 —
속을 보면 `LineRenderer`와 같은 띠 생성기라 **꺾일 때 이음매가 무너지는 문제가 그대로 돌아오고**,
점이 시간으로 사라져 "점령할 때까지 유지"와 맞지 않으며, **도화선이 태운 중간 구간만 빼는 것이 불가능하고**(2-14-1),
어디로든 신발로 반대편에 나타나면 맵을 가로지르는 선이 그어진다. 지금 구조가 더 많이 할 수 있다.

| 무엇 | 어디 |
|---|---|
| 굵기 · 발광 켬/끔 | `GameConfig`(2-9) — 에셋에서 바로 |
| 색(머리 · 꼬리 · 불 · 발광) | `CGridRenderer`의 상수 — 시각 정체성이라 설정이 아니라 코드에 둔다(`CUI_InGame`의 플래시 색과 같은 자리) |
| 그라디언트 · UV 생성 | `CTrailMesh_Utility.CTrailStyle` |
| 셰이더 | `Assets/Shader/Shd_Trail_Neon.shader` → 재질은 `Resources/Mat_Trail.mat` |

- **꼬리 → 머리로 색이 흐른다.** 플레이어 쪽이 밝고, 선이 시작된 쪽은 옅다 — 어디로 가고 있는지가 색으로 읽힌다.
  색은 **선 전체에서의 위치**로 섞으므로 토막(탄 구간을 뺀 앞뒤 · 불 머리)이 여럿이어도 한 줄기로 이어져 보인다
- **발광은 같은 메시에 한 겹 더** 깐다(`GLOW_WIDTH_SCALE` 2.4배 · 알파 0.25). 한 메시 안에서는 넣은 순서대로 그려지므로
  발광을 먼저, 본 선을 나중에 넣는다. 새 오브젝트도 새 드로우콜도 늘지 않는다
- **셰이더는 `Resources/Mat_Trail.mat`으로 갈아 끼운다** — 있으면 그 재질로 그리고, 없으면 흰 단색으로 그린다.
  메시가 꼭짓점 색과 UV를 같이 넘기므로 흐르는 무늬 · 번쩍임 · 외곽선을 셰이더에서 만들 수 있다.
  **u = 지나온 거리(굵기 한 칸이 1이라 무늬가 굵기에 맞춰 반복된다) · v = 띠를 가로지르는 0~1**
- 가져온 재질은 우리가 만든 것이 아니므로 `Release`에서 파괴하지 않는다(`m_bOwnMaterial`)

#### 260924_네온 셰이더 (`Shd_Trail_Neon.shader`)
기본으로 붙는 셰이더다. **재질(`Mat_Trail.mat`)은 `Setup Assets`가 만든다** — 스프라이트 · UI 프리팹과 같은
산출물이라 리포에 없다(3-1). **이미 있으면 덮어쓰지 않는다** — 인스펙터에서 맞춰 둔 값이 날아가면 안 된다.

- 네온은 "가운데는 하얗게 타고 가장자리로 갈수록 색만 번진다"가 핵심이라 **v로 심지와 번짐을, u로 흐르는 빛을** 만든다
- 기본 혼합은 **가산**(`Blend SrcAlpha One`) — 어두운 가림막 위에서 빛나 보인다. 밝은 보상 그림 위에서 너무
  날아가면 **재질에서 Dst를 `OneMinusSrcAlpha`로** 바꾸면 보통 반투명이 된다(재질 속성으로 열어 뒀다)
- 꼭짓점 색을 그대로 쓰므로 **불 머리(주황)도 같은 셰이더로 네온이 된다** — 색만 다르게 들어올 뿐이다
- `Cull Off`다. 띠는 양면이라 감기 방향이 뒤집혀도 사라지지 않는다(2-3의 앞면 순서 주의사항과 같은 자리의 안전망)
- 셰이더가 컴파일되는지는 배치모드에서 `ShaderUtil.GetShaderMessageCount`로 확인했다(에러 0 · isSupported)
...
| `m_bTrailGlowEnabled` | 260924_선 뒤에 옅은 발광 띠를 한 겹 더 깐다 |
