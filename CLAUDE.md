...
- **카드 문턱 기록(`m_iRatioStep`)도 같이 지운다** — 안 그러면 점령률이 0으로 돌아가 2·3웨이브에는
  카드가 아예 안 나온다. `CARD_RATIO_STEP`(5%)이 웨이브마다 다시 처음부터 돈다(2-21)
...
```
CStage_Manager.Check_CardReady  점령률이 CARD_RATIO_STEP(5%)을 하나 더 넘었나 → OnCardReady (2-21)
...
>   보호막이 없고 `bLineCut == false`일 때만 회피 판정을 본다. **선이 끊긴 것(몬스터가 선에 닿음 · 자기 선 밟기)은
...
### 2-21. 3지선다 트리거 — 다시 점유율 자동 (260920 조각 게이지 → 260923 다시 점유율)
...
> 쪽이 더 직접적이다.**
> - `CStage_Manager.CARD_RATIO_STEP`(5%, 코드 상수 — 맵마다 다르게 줄 이유가 아직 없어 CSV로 빼지 않았다).
>   점유율이 이 폭을 하나 더 넘을 때마다 3지선다를 하나 연다(`Check_CardReady`가 웨이브당 `m_iRatioStep`으로
>   지금까지 넘은 문턱 수를 센다)
