...
#### 시작 테두리는 덮어 둔다 (260912)
`iBorderThick`(2칸)은 플레이어가 처음 설 안전 지대다. **없앨 수 없다** —
점령은 딴 선을 점령지에 다시 붙여야 성립하므로, 시작 점령지가 없으면 아무것도 딸 수 없다.

대신 **가림막에 구멍을 내지 않는다.** 예전에는 시작하자마자 보상이 테두리처럼 비쳐
'드러내는 재미'가 먼저 새어 나갔다. `CTerritoryGrid.Is_StartOwned`로 구분하고
`CGridRenderer.Get_PixelColor`가 그 칸만 가림막 색으로 칠한다.

- **안전 지대인 것은 그대로다.** 점령률 분모/분자도 예전과 같다
- **플레이어가 직접 딴 땅만 뚫린다**
- 웨이브를 넘길 때의 REVEAL 연출에서는 가림막 전체가 투명해지므로 최종 그림은 다 보인다
