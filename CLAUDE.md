...
- 260923_**`Capture`는 넓이를 보지 않는다 — 내가 두른 안쪽을 먹는다(올가미 규칙).**
  선이 나간 자리와 돌아온 자리를 내 땅 경계를 따라 **짧은 쪽으로** 이으면 닫힌 고리가 되고, 그 안쪽이 점령지가 된다
  (`Try_Capture_Lasso`). 먼 길로 이으면 상관없는 빈 땅까지 고리 안에 들어오므로 짧은 쪽을 쓴다.
  - 예전에는 빈 땅 조각 중 **가장 넓은 것을 남기고 나머지를 먹었다**(= 사실상 작은 쪽을 먹는 규칙).
    그런데 **두른 쪽이 남은 땅보다 넓어지면 정반대로 뒤집힌다** — 크게 휘두를수록 엉뚱한 곳이 점령됐고,
    웨이브 목표가 60~70%면 매 웨이브 후반에 반드시 걸리는 조건이라 "과감하게 하면 배신당하는" 판이 됐다.
    원조 Qix · 볼피드도 "작은 쪽"이 아니라 **"적이 없는 쪽"** 을 먹는다 — 넓이는 원래 기준이 아니었다
  - **선이 끊겼거나(어디로든 신발) 양 끝이 다른 섬에 붙었으면** 고리를 만들 수 없어 예전 규칙(가장 넓은 조각만 남기기)으로 떨어진다
  - 가둬서 죽이기는 그대로다 — 두른 안쪽에 있던 몬스터는 점령과 함께 죽는다
...
- 260923_**누른 방향이 변과 직각이면 경계를 따라 돈다**(260902_선분 자동 추적을 다각형으로 옮긴 것).
  "아래로 가려고 누르고 있으면 좌우 경계도 선을 따라 알아서 간다"가 이것이다. 따라가다 그 방향으로
  나갈 수 있게 되면 `Can_Exit`이 받아 저절로 나간다. 어느 쪽으로 돌지는 이 순서로 정한다(260924).
  - ① 조금이라도 기운 쪽(`FOLLOW_MIN_DOT` 0.02) — 사선 경계에서 걸리지 않게
  - ② 돌던 방향(`m_iFollowSign`) — 한 번 돌기 시작하면 **모서리를 돌아서도 계속 간다**. 예전에는 모서리에서 멈춰
    "점령지에서 자꾸 막힌다"가 됐다
  - ③ 마지막으로 나아가던 방향이 변에 걸쳐 있으면 그것 — 예전 칸 시절이 `m_eCurDir`로 하던 것과 같다
  - ④ 그래도 모르면 **경계를 앞뒤로 6칸 내다보고 누른 방향에 더 가까워지는 쪽**(`Find_FollowSign`).
    점령하고 막 돌아온 직후가 늘 이 경우다 — 들어온 방향이 경계와 직각이라 ①~③이 전부 0이 된다
  - ⑤ 양쪽이 똑같으면(곧은 변 한가운데서 벽을 미는 격) 그 자리에 선다
  - **"누른 방향으로 나갈 수 있는 가장 가까운 자리를 찾아간다"는 안은 버렸다** — 테두리 위에서 안쪽을 누르면
    맵을 스물몇 칸이나 돌아 반대편까지 달려가 버렸다. 따라가기는 가던 길을 잇는 것이지 길을 찾아 주는 것이 아니다
  - 260924_경계 위 여러 지점에서 네 방향을 눌러 세어 보니 **막히는 입력이 점령 직후 25% · 경계를 타던 중 9%**였다.
    위 규칙을 넣은 뒤 둘 다 0%다(`CProtoTest`의 따라가기 테스트가 이 규칙들을 하나씩 잡아 둔다)
...
- `CGridRenderer`는 점령지 다각형을 마스크 텍스처에 찍어 가림막을 뚫는다. **셰이더 없음.**
  ...
  **선은 텍스처가 아니라 띠 메시로 따로 그린다**(`CTrailMesh_Utility`, 타는 구간은 빼고 불꽃 머리를 붙인다).
  260924_굵기는 `GameConfig.m_fTrailWidthCell`(기본 0.3칸) — **보이기만 하는 값이라 점령 판정과 무관하다**(점령은 0.01칸 띠로 계산한다).
  260923_처음에는 `LineRenderer`를 썼는데 **구간이 굵기보다 짧으면 이음매가 무너진다** — 한 프레임에 0.15칸씩 긋는데
  굵기가 그보다 커서 꺾은 직후 서너 프레임이 늘 그 상태라, 꺾을 때마다 선이 한순간 일그러져 보였다.
...
| `m_fTrailWidthCell` | 260924_긋는 중인 선의 굵기(칸, 기본 0.3). 보이기만 하는 값이라 점령 판정과 무관하다(2-3) |
