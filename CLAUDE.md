...
- 260921_**선을 긋는 중에도 손을 떼면 멈춘다.** 예전에는 미점령 지대에서 멈출 수 없어 가던 방향으로 저절로 나아갔다 —
  조작하지 않은 움직임으로 죽는 일이 잦았다. 뒤로 꺾는 입력처럼 막힌 방향을 눌러도 그 자리에 선다(`CMoveHandler.Try_StartMove`)
...
- 260921_**한 번에 한 장만 연다**(`CPickQueue`). 전체 자석으로 조각을 한꺼번에 주우면 게이지가 여러 번 넘치는데,
  예전에는 창을 연달아 열어 앞 창이 남아 입력을 전부 막았다. 넘친 만큼 쌓아 두고, 고른 뒤 `CStage_Manager.Close_Pick`이
  다음 창을 연다. 선택지가 없어 못 열었을 때도 `Close_Pick`을 불러야 대기열이 막히지 않는다
...
> - 260922_**선이 끊긴 것(몬스터가 선에 닿음 · 자기 선 밟기)은 회피로 흘리지 못한다**(`Lose_Life(bLineCut)`). 회피가 높으면
>   몬스터가 선을 지나가도 멀쩡해 규칙이 사라진 것처럼 보였다(강화 · 가방 · 카드가 쌓이면 50%를 넘는다). 보호막 · 무적은 그대로 막는다
...
#### 선을 긋는 동안은 칸이 아니라 각도로 움직인다 (260921, `CMoveHandler_Free`)
칸 단위로만 움직이면 대각선은 계단, 나선은 네모가 되어 **그 모양으로 점령할 수 없었다.**
그래서 8방향 · 나선형은 **선을 긋는 동안만** 몸이 자유 각도로 움직이고, 지나간 칸을 순서대로 `Step_To`에 넘긴다.
규칙 단일 진입점(2-3)은 그대로다 — 점령한 모양이 사선 · 원이 되고, 가장자리만 칸 크기 계단으로 남는다.

- **지나간 칸은 반드시 4방향으로 이어지게 고른다**(`Advance_Free`). 모서리를 스칠 때는 더 많이 넘어간 축부터
  밟는다 — 대각선으로만 이어지면 플러드필이 그 틈으로 새어 나간다(아래 '가로 한 칸 + 세로 한 칸'과 같은 이유)
- 한 프레임에 칸을 여럿 지날 수 있어 대기열로 쌓고, `CPlayer`가 같은 프레임에 전부 판정한다(`Try_PopArrived`).
  점령 · 사망으로 자리를 옮기면(`Snap_To` · `Teleport`) 남은 칸은 버린다
- 맵 끝 · 잘린 칸 · 방금 떠나온 칸으로는 들어가지 않고 그 축만 멈춘다(벽을 따라 미끄러진다)
- **뒤로 꺾는 입력(135도 이상)은 무시한다** — 자기 선을 밟는 즉사를 막는다(칸 이동의 `Try_Get_PrevTrailCell`과 같은 뜻)
- 내 땅 경계선 위에서는 예전처럼 칸 단위다 — 경계선이 곧 칸의 줄이라서다. 선이 끝나면(점령 · 사망) 칸 이동으로 돌아온다
- 워프 · 마감이 이어 쓸 수 있게 `CUR_DIR`은 마지막으로 밟은 칸의 4방향으로 남긴다

#### 대각선은 '가로 한 칸 + 세로 한 칸'으로 밟는다 (경계선 위)
...
- 260921_**몸은 두 칸을 비스듬히 한 번에 미끄러진다**(`CMoveHandler.WORLD_POS`의 `m_iDiagPhase`). 칸을 따라 그리면
  좌우로 꺾이는 지그재그로 보여 대각선으로 읽히지 않았다. 규칙(선 · 판정)은 여전히 두 칸이다
- `Can_Move`는 **대각선 방향을 항상 거절한다.** 한 칸짜리 대각선 이동은 설계상 존재하지 않는다
- 8방향 캐릭터의 `fSpeedRateBase`는 0.72다. 자유 이동은 방향을 정규화하므로 대각선이 더 빠르지 않다 —
...
- 260921_**점령 조각은 방금 닫은 도형 두 칸 바깥에 한 무더기로 떨어진다**(`CTerritoryGrid.Collect_CaptureFrontier` →
  `CStage_Manager.Pick_ShardCluster`). 맵 아무 데나 뿌리면 줍기가 '먼 곳까지 걸어가는 일'이었다 — 먹은 자리 옆에 두면
  점령과 줍기가 한 흐름이 되고, 줍는 것은 짧고 위험한 한 번의 돌진이 된다. 딱 붙이지 않는 것은 경계를 걷기만 해도 주워지지 않게다
- 260921_**가둬 죽인 몬스터는 한 마리당 조각을 곧바로 준다**(`MapInfo.csv`의 `iShardPerTrapKill`, 지금 3). 쓰러진 자리의 보통 조각은 그대로 떨어진다
