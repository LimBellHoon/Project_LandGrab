...
지금 있는 8종 중 실제로 코드가 붙은 건 5종뿐이다. 나머지 셋은 **더 큰 새 시스템이
먼저 필요해서** 의도적으로 미뤘다(아래).
...
| 영혼 수집가 | 🔲 | 맵에 무작위로 떨어뜨리고 주울 대상(영혼)이 아직 없다 — 픽업 오브젝트를 새로 만들어야 한다(기믹 소환물과 같은 자리, `CStage_Manager`가 소유) |
...
로컬에서 만들어지는 산출물이라(3-1) 클라우드 세션에서는 실제로 떠 보이는지 확인할 방법이
없다 — UI 쪽 일반화는 로컬에서 이어서 볼 것.
