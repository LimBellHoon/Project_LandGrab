...
- 입력이 없으면 가던 방향으로 계속 가고, 변 길이만큼 가면 시계 방향으로 한 번 꺾는다.
  260921_**변은 두 번 꺾을 때마다 2칸씩 길어진다**(`Get_SpiralLeg` — 4, 4, 6, 6 …). 고정 4칸이면 같은 네모를 돌아
  작은 네모만 점령하고 끝났다. 2칸씩 늘면 고리 사이가 한 칸 비어 자기 선을 밟지 않는다. 안전 지대에서는 늘지 않는다
- 260921_**입력은 새로 누른 순간만 본다**(`m_eSpiralInput`). 누르고 있는 방향이 매 칸 이기면 자동 회전이 바로 되돌려져 소용돌이가 안 생겼다
