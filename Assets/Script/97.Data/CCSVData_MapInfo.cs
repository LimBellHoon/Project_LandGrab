...
namespace Client
{
    ...
    public class CCSVData_MapInfo : CCSVData
    {
        ...
        protected override void Parse_CSVData(string[] strField)
        {
            ...
            {
                iMapID          = CCsvUtil.To_Int(strField, 0),
                strMapName      = CCsvUtil.To_String(strField, 1),
                iGridWidth      = CCsvUtil.To_Int(strField, 2, 60),
                iGridHeight     = CCsvUtil.To_Int(strField, 3, 100),
                fCellSize       = CCsvUtil.To_Float(strField, 4, 0.12f),
                iBorderThick    = CCsvUtil.To_Int(strField, 5, 2),
                iLife           = CCsvUtil.To_Int(strField, 6, 3),
                iWaveCount      = CCsvUtil.To_Int(strField, 7, 1),
                strShapeMask    = CCsvUtil.To_String(strField, 8),
            };
            ...
            cInfo.lstLayerTex = CCsvUtil.To_List(strField, 9);

            Parse_Wave(cInfo, strField);

            // 표가 조용히 어긋나면 스테이지가 통째로 이상해지므로 여기서 전부 잡는다.
            cInfo.IS_VALID =
                CCsvUtil.Check_Count(TABLE_NAME, cInfo.iMapID, "strLayerTex",
                                     cInfo.lstLayerTex.Count, cInfo.iWaveCount + 1)
              & CCsvUtil.Check_Count(TABLE_NAME, cInfo.iMapID, "strWaveEnemy",
        ...
        /// </summary>
        private static void Parse_Wave(CMapInfo cInfo, string[] strField)
        {
            List<string> lstWaveEnemy = CCsvUtil.To_List(strField, 10);
            List<float>  lstRatio     = CCsvUtil.To_FloatList(strField, 11);
            List<float>  lstTime      = CCsvUtil.To_FloatList(strField, 12);

            for (int i = 0; i < lstWaveEnemy.Count; ++i)
            {
                CWaveInfo cWave = new CWaveInfo
                {
                    fClearRatio = Get_OrLast(lstRatio, i, 0.7f),
                    fTimeLimit  = Get_OrLast(lstTime, i, 180f),
                };
        ...
        {
            string[] arrToken = strWave.Split(CCsvUtil.SPLIT_ITEM);
            ...
            for (int i = 0; i < arrToken.Length; ++i)
            {
                ...
                string[] arrPair = strToken.Split(CCsvUtil.SPLIT_COUNT);
                CWaveEnemy cEnemy = new CWaveEnemy
                {
                    iEnemyID = CCsvUtil.To_Int(arrPair, 0),
                    iCount   = arrPair.Length > 1 ? CCsvUtil.To_Int(arrPair, 1, 1) : 1,
                };
        ...
        private static float Get_OrLast(List<float> lstValue, int iIndex, float fDefault)
        {
            if (lstValue.Count == 0)
                return fDefault;
